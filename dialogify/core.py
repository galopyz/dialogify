...
def get_sections(main):
    "Get all direct child sections as (h2_title, section_element) tuples"
...
def create_msgs(doc_tuples, **kwargs):
    """Create solveit messages from list of (msg_type, content) tuples"""
    for msg_type, content in doc_tuples: add_msg(content=content, msg_type=msg_type, placement='at_end', **kwargs)

# %% ../nbs/00_core.ipynb 35
def mk_dialog(url, dname=''):
    """Fetch Python docs URL and create solveit messages from it"""
    if not Path(f'{dname}.ipynb').exists(): Path(f'{dname}.ipynb').write_json({"cells":[],"metadata":{},"nbformat":4,"nbformat_minor":5})
